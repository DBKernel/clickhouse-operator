...
def delete_all_chi(ns=namespace):
    ...
    if "clickhouseinstallations.clickhouse.dbkernel.com" in crds:
...
def create_and_check(config, check, ns=namespace, timeout=600):
    ...
    if "pod_count" in check:
        wait_object("pod", "", label=f"-l clickhouse.dbkernel.com/chi={chi_name}", count=check["pod_count"], ns=ns)
...
def wait_objects(chi, object_counts, ns=namespace):
    with Then(
        ...
        for i in range(1, max_retries):
            cur_object_counts = count_objects(label=f"-l clickhouse.dbkernel.com/chi={chi}", ns=ns)
...
def get_pod_spec(chi_name, ns=namespace):
    pod = get("pod", "", ns=ns, label=f"-l clickhouse.dbkernel.com/chi={chi_name}")["items"][0]
    return pod["spec"]
...
def get_pod_names(chi_name, ns=namespace):
    pod_names = launch(
        f"get pods -o=custom-columns=name:.metadata.name -l clickhouse.dbkernel.com/chi={chi_name}",
        ns=ns,
    ).splitlines()
    return pod_names[1:]
        ...
        "requiredDuringSchedulingIgnoredDuringExecution": [
            {
                "labelSelector": {
                    "matchLabels": {
                        "clickhouse.dbkernel.com/app": "chop",
                        "clickhouse.dbkernel.com/chi": f"{chi_name}",
                        "clickhouse.dbkernel.com/namespace": f"{ns}",
                    },
                },
